        ...
        foreach (KeyValuePair<Vector2Int, ChunkMeta> c in WorldMeta)
        {
            ...
            foreach (KeyValuePair<Vector2Int, HexMeta> h in currentChunk.HexMeta)
            {
                ...
                foreach (KeyValuePair<Vector2Int, VertexMeta> v in currentHex.VertexMeta)
                {
                    ...
                    hexUVs.Add(new Vector2(currentVertex.RealPos.x / (maxX - minX), currentVertex.RealPos.y / (maxY - minY)));
                    vertexCount++;
                }
            ...
            CreateChunk(currentChunk, chunkVertices.ToArray(), chunkTriangles.ToArray(), chunkUVs.ToArray());
        }
    }

    private void CreateChunk(ChunkMeta chunk, Vector3[] vertices, int[] triangles, Vector2[] uvs)
    {
        Mesh mesh = new Mesh();
        ...
        meshObject.transform.localPosition = new Vector3(chunk.RealPos.x, 0f, chunk.RealPos.y);
